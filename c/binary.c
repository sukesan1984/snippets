...
/*   Updated: 2020/12/05 16:43:43 by ktakami          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bmp	initialize()
{
	...
	bmp_data.filesize = create_data(126, 4);
	bmp_data.prepared = create_data(0, 4);
	bmp_data.offset = create_data(62, 4);
	bmp_data.header = create_data(40, 4);
	bmp_data.width	= create_data(16, 4);
	bmp_data.height	= create_data(16, 4);
	bmp_data.plane = create_data(1, 2);
	bmp_data.color_bit = create_data(1, 2);
	bmp_data.comptype = create_data(0, 4);
	bmp_data.img_size = create_data(0x40, 4);
	bmp_data.res_width = create_data(0x10, 4);
	bmp_data.res_height = create_data(0x10, 4);
